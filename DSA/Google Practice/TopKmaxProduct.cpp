...
int getProductFromPositive(const vector<int>& nums, int k) {
    int product = 1;
    for(int end = nums.size() - 1; k; end--, k--) product *= nums[end];
    return product;
}

int getProductFromNegative(const vector<int>& nums, int k) {     
    if(k % 2) return getProductFromPositive(nums, k);
    int product = 1;
    for(int start = 0; k; k--, start++) product *= nums[start];
    return product;
}
...
int maxProduct(vector<int>& nums, int& k) {
    if(nums.size() < k) return INT_MIN; // can't select k elements
    
    auto [product, pos, neg] = getProduct(nums);
    if(nums.size() == k) return product;
    
    sort(nums.begin(), nums.end());
    
    if(pos == nums.size()) return getProductFromPositive(nums, k);
    if(neg == nums.size()) return getProductFromNegative(nums, k);
